 ...
 * @author rayz
 */
public class ExpiredMayorsPermit extends javax.swing.JInternalFrame {

    public static String mtop_id = "";
    static Connection conn = null;

    public ExpiredMayorsPermit() {
    ...
    public static void view() {
        String search[][] = null;
        String col[] = {"MTOP NO", "NAME", "ADDRESS", "MAKE", "PLATE NO", "CHASSIS NO", "MOTOR NO", "REG. NO", "MP EXPIRATION DATE"};

        try {
            conn.setAutoCommit(true);
            Statement s = (Statement) conn.createStatement();
            s.executeQuery("SELECT v.MTOP_NO, CONCAT(lastname,', ',firstname) as fullname, address, plate_no, make, chassis_no, motor_no, reg_no, DATE_FORMAT(mp_expiration_date,'%b %d %Y') as exp FROM vehicle v, operator o WHERE v.MTOP_NO = o.MTOP_NO AND mp_expiration_date <= NOW() AND status != 'DROP' ORDER BY v.MTOP_NO");
            ResultSet rs1 = s.getResultSet();//manages result
            int count = 0;
            s = (Statement) conn.createStatement();
            ResultSet res = s.executeQuery("SELECT count(v.MTOP_NO) as num, CONCAT(lastname,', ',firstname) as fullname, address, plate_no, make, chassis_no, motor_no, reg_no, DATE_FORMAT(mp_expiration_date,'%b %d %Y') as exp FROM vehicle v, operator o WHERE v.MTOP_NO = o.MTOP_NO AND mp_expiration_date <= NOW() AND status != 'DROP' ORDER BY v.MTOP_NO");//manages result
            while (res.next()) {
    ...
    private void initComponents() {
        ...
        setTitle("List of Expired Mayor's Permit");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        try {
            String report = "reports/expired_mayors_permit.jrxml";
            JasperReport jr = JasperCompileManager.compileReport(report);
            JasperPrint jp = JasperFillManager.fillReport(jr, null,conn);
            JasperViewer jv = new JasperViewer(jp, false);
            jv.setTitle("List of Expired Mayors Permit");
            jv.setVisible(true);
        } catch (Exception e) {
