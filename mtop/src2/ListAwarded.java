 ...
 * @author rayz
 */
public class ListAwarded extends javax.swing.JInternalFrame {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try {
            ...
            map.put("to_date", to2);
            String report = "reports/list_awarded_mtop.jrxml";
            JasperReport jr = JasperCompileManager.compileReport(report);
            JasperPrint jp = JasperFillManager.fillReport(jr, map, conn);
            JasperViewer jv = new JasperViewer(jp, false);
            jv.setTitle("List of Awarded MTOP");
            jv.setVisible(true);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try {
            ...
            s.executeQuery("SELECT MTOP_NO, CONCAT(firstname,' ',lastname) as fullname, plate_num, make, chassis_no, motor_no, ddate, type, action FROM transaction_logs WHERE ddate >= '" + from2 + "' AND ddate <= '" + to2 + "' AND type = 'Awarding' ORDER BY ID DESC");
            ResultSet rs1 = s.getResultSet();//manages result
            int count = 0;
            s = (Statement) conn.createStatement();
            ResultSet res = s.executeQuery("SELECT count(MTOP_NO) as num, CONCAT(firstname,' ',lastname) as fullname, plate_num, make, chassis_no, motor_no, type, action FROM transaction_logs WHERE ddate >= '" + from2 + "' AND ddate <= '" + to2 + "' AND type = 'Awarding' ORDER BY ID DESC");//manages result
            while (res.next()) {
    ...
    private void searchfieldKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_searchfieldKeyReleased
        ...
        try {
            conn.setAutoCommit(true);
            Statement s = (Statement) conn.createStatement();
            s.executeQuery("SELECT MTOP_NO, CONCAT(firstname,' ',lastname) as fullname, plate_num, make, chassis_no, motor_no, ddate, type, action FROM transaction_logs WHERE ddate >= '" + from2 + "' AND ddate <= '" + to2 + "' AND type = 'Awarding' AND CONCAT(MTOP_NO,', ',lastname,', ',firstname,', ',plate_num) RLIKE '" + searchfield.getText() + "' ORDER BY ID DESC");
            ResultSet rs1 = s.getResultSet();//manages result
            int count = 0;
            s = (Statement) conn.createStatement();
            ResultSet res = s.executeQuery("SELECT count(MTOP_NO) as num, CONCAT(firstname,' ',lastname) as fullname, plate_num, make, chassis_no, motor_no, type, action FROM transaction_logs WHERE ddate >= '" + from2 + "' AND ddate <= '" + to2 + "' AND CONCAT(MTOP_NO,', ',lastname,', ',firstname,', ',plate_num) RLIKE '" + searchfield.getText() + "' AND type = 'Awarding' ORDER BY ID DESC");//manages result
            while (res.next()) {
                count = res.getInt("num");
            }
