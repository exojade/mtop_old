 ...
 * @author rayz
 */
public class ForReleaseMTOP extends javax.swing.JInternalFrame {

    public static String mtop_id = "";
    static Connection conn = null;

    public ForReleaseMTOP() {
    ...
    public static void view() {
        String search[][] = null;
        String col[] = {"MTOP NO", "NAME", "ADDRESS", "MAKE", "PLATE NO", "CHASSIS NO", "MOTOR NO", "REG. NO", "STATUS"};

        try {
            conn.setAutoCommit(true);
            Statement s = (Statement) conn.createStatement();
            s.executeQuery("SELECT vehicle.MTOP_NO, CONCAT(lastname,', ',firstname) as fullname, address, plate_no, make, chassis_no, motor_no, reg_no, status, released_date, expiration_date FROM vehicle JOIN operator ON vehicle.MTOP_NO = operator.MTOP_NO WHERE status ='For Release' ORDER BY vehicle.MTOP_NO");
            ResultSet rs1 = s.getResultSet();//manages result
            int count = 0;
            s = (Statement) conn.createStatement();
            ResultSet res = s.executeQuery("SELECT count(vehicle.MTOP_NO) as num FROM vehicle JOIN operator ON vehicle.MTOP_NO = operator.MTOP_NO WHERE status ='For Release' ORDER BY vehicle.MTOP_NO");//manages result
            while (res.next()) {
    ...
    private void initComponents() {
        ...
        setTitle("List of For Release MTOP");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        try {
            String report = "reports/for_release.jrxml";
            JasperReport jr = JasperCompileManager.compileReport(report);
            JasperPrint jp = JasperFillManager.fillReport(jr, null,conn);
            JasperViewer jv = new JasperViewer(jp, false);
            jv.setTitle("List for Release MTOP");
            jv.setVisible(true);
    ...
    private void searchfieldKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_searchfieldKeyReleased
        ...
        try {
            conn.setAutoCommit(true);
            Statement s = (Statement) conn.createStatement();
            s.executeQuery("SELECT vehicle.MTOP_NO, CONCAT(lastname,', ',firstname) as fullname, address, plate_no, make, chassis_no, motor_no, reg_no, DATE_FORMAT(expiration_date,'%b %d %Y') as exp FROM vehicle JOIN operator ON vehicle.MTOP_NO = operator.MTOP_NO WHERE expiration_date <= NOW() AND status = 'For Release' AND CONCAT(vehicle.MTOP_NO,', ',lastname,', ',firstname,', ',address,', ',plate_no) RLIKE '" + searchfield.getText() + "' ORDER BY vehicle.MTOP_NO");
            ResultSet rs1 = s.getResultSet();//manages result
            int count = 0;
            s = (Statement) conn.createStatement();
            ResultSet res = s.executeQuery("SELECT count(vehicle.MTOP_NO) as num, CONCAT(lastname,', ',firstname) as fullname, address, plate_no, make, chassis_no, motor_no, reg_no, DATE_FORMAT(expiration_date,'%b %d %Y') as exp FROM vehicle JOIN operator ON vehicle.MTOP_NO = operator.MTOP_NO WHERE expiration_date <= NOW() AND status = 'For Release' AND CONCAT(vehicle.MTOP_NO,', ',lastname,', ',firstname,', ',address,', ',plate_no) RLIKE '" + searchfield.getText() + "' ORDER BY vehicle.MTOP_NO");//manages result
            while (res.next()) {
                count = res.getInt("num");
            }
