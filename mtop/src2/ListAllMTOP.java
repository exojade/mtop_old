 ...
 * @author rayz
 */
public class ListAllMTOP extends javax.swing.JInternalFrame {
    ...
    public static void view() {
        String search[][] = null;
        String col[] = {"MTOP NO", "NAME", "ADDRESS", "MAKE", "PLATE NO", "CHASSIS NO", "MOTOR NO", "REG. NO", "STATUS", "RELEASED", "EXPIRATION", "GENDER"};

        try {
            conn.setAutoCommit(true);
            Statement s = (Statement) conn.createStatement();
            s.executeQuery("SELECT vehicle.MTOP_NO, CONCAT(lastname,', ',firstname) as fullname, address, plate_no, make, chassis_no, motor_no, reg_no, status, released_date, expiration_date, Gender FROM vehicle JOIN operator ON vehicle.MTOP_NO = operator.MTOP_NO ORDER BY vehicle.MTOP_NO");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        try {
            String report = "reports/list_all_mtop.jrxml";
            JasperReport jr = JasperCompileManager.compileReport(report);
            JasperPrint jp = JasperFillManager.fillReport(jr, null, conn);
            JasperViewer jv = new JasperViewer(jp, false);
            jv.setTitle("List of All MTOP");
            jv.setVisible(true);
        } catch (Exception e) {
            e.printStackTrace();
        }
